...
from socnet_entity.utils.exceptions import ProxyValidationError, InstagramValidationError, TelegramValidationError
from socnet_entity.utils.serializer import EnumSerializer
...
def validate_telegram_api_id(api_id: str | int) -> None:
    ...
    if not (10000 <= api_id <= 999999999):
        raise TelegramValidationError("Telegram api_id must be in the valid range (10000–9999999)")
...
def validate_telegram_api_hash(api_hash: str) -> None:
    ...
    if not re.fullmatch(r"[0-9a-fA-F]{32}", api_hash):
        raise TelegramValidationError("Telegram api_hash must be a 32-character hex string")


def validate_telegram_session_token(token: str) -> None:
    if not isinstance(token, str) or not token.strip():
        raise TelegramValidationError("Telegram session token must be a non-empty string")
