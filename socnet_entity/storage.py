...
from socnet_entity.utils.validators import resolve_enum
...
class Storage:
        ...
        try:
            if not self.path.exists():
                ...
                if default_data is not None and not self._is_serializable(default_data):
                    raise ValueError("default_data должен быть dataclass или коллекцией dataclass-ов")
                self.save(default_data if default_data is not None else {})
                return self.load() if default_data is not None else {}
            else:
                return self.load()
        except Exception as e:
            raise RuntimeError(f"Ошибка при обработке файла {self.path}: {e}") from e
    ...
    def save(self, data: Any) -> None:
        try:
            ...
            raise RuntimeError(f"Ошибка при сохранении в файл {self.path}: {e}") from e
    ...
    def load(self) -> Any:
        try:
            ...
            raise RuntimeError(f"Ошибка при загрузке из файла {self.path}: {e}")
    ...
    def _dict_to_dataclass(cls, data: dict) -> Any:
        ...
        if not dataclass_type:
            raise ValueError(f"Неизвестный тип dataclass: {type_name}")
