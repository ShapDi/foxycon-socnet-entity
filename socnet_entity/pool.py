...
from socnet_entity.utils.exceptions import StorageLoadError, StorageSaveError, EntityPermanentlyBlockedError
...
class EntityPool:
    def __init__(self, path: Optional[str] = None):
        ...
        if path:
            try:
                ...
                raise StorageLoadError(f"Ошибка инициализации Storage по пути {path}: {e}") from e

    def add(self, entity: Entity):
        if any(self._is_duplicate(entity, existing) for existing in self._entities.values()):
            logging.warning(f"Сущность {entity} уже существует и не может быть добавлена повторно")
            return
    ...
    def get_available_of_type(self, t: Type[T]) -> list[T]:
        return [
            e.try_as(t)
            for e in self._entities.values()
            if e.status == Status.WORKING and e.id not in self._in_use and e.is_of_type(t)
        ]
    ...
    def check_entity(self, entity: Entity):
        if entity.status == Status.PERMANENTLY_BLOCKED:
            raise EntityPermanentlyBlockedError(f"Entity {entity.id} is permanently blocked")
    ...
    def _is_duplicate(self, new_entity: Entity, existing_entity: Entity) -> bool:
        ...
        return all(getattr(new_entity, f) == getattr(existing_entity, f) for f in fields)
    ...
    def _save(self):
        ...
        try:
            ...
            raise StorageSaveError(f"Ошибка при сохранении: {e}") from e
